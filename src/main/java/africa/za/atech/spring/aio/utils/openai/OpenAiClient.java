...
public class OpenAiClient {
    ...
    private static Map<String, String> setHeaders(String referenceId, Assistants assistant) {
        ...
        headers.put("Authorization", "Bearer " + assistant.getLlmApiKey());
        headers.put("OpenAI-Organization", assistant.getLlmOrganisationId());
    ...
    public static Response addMessage(String referenceId, String threadId, Assistants assistant, Role role, String content) {
        ...
        req.setBaseUri(url);
        req.addHeaders(setHeaders(referenceId, assistant));
        req.setBody(new AddMessageToThread(role, content));
    ...
    public static Response runThread(String referenceId, String threadId, Assistants assistant, String additionalInstruction) {
        ...
        req.setBaseUri(url);
        req.addHeaders(setHeaders(referenceId, assistant));
        if (assistant.getAdditionalInstructions() == null) {
            assistant.setAdditionalInstructions("");
        }
        req.setBody(new RunThreadMessage(assistant.getLlmAssistantId(), (assistant.getAdditionalInstructions() + " " + additionalInstruction).trim()));
